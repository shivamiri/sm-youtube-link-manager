...
// Authentication is checked in header.php, but we ensure it's loaded safely
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'update_channel') {
        ...
        if (empty($msg)) {
            ...
            if ($stmt->execute()) {
                redirect('index.php?msg=channel_updated');
            } else {
    ...
    if ($action === 'update_security') {
        ...
        if (password_verify($old_pass, $res['password'])) {
            ...
            if ($upd->execute()) {
                redirect('index.php?msg=security_updated');
            }
            $upd->close();
        } else {
...
if (isset($_GET['msg'])) {
    ...
    if ($_GET['msg'] === 'channel_updated') { $msg = 'Channel settings updated.'; $msg_type = 'success'; }
    if ($_GET['msg'] === 'video_saved') { $msg = 'Video saved successfully.'; $msg_type = 'success'; }
    if ($_GET['msg'] === 'security_updated') { $msg = 'Security credentials updated.'; $msg_type = 'success'; }
}
...
// Refresh settings after possible update
    ...
    <div class="lg:col-span-1 space-y-8">
        ...
        <div class="bg-card p-6 rounded-2xl border border-gray-800 shadow-lg" id="video-form">
            <div class="flex justify-between items-center border-b border-gray-700 pb-4 mb-4">
                ...
                <?php if ($edit_video): ?>
                    <a href="index.php" class="text-sm text-gray-400 hover:text-white">Cancel Edit</a>
                <?php endif; ?>
            </div>
            ...
            <form method="POST" action="index.php" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    <label class="block text-sm text-gray-400 mb-1">Video Title (Leave blank for auto-fetch)</label>
                ...
                <button type="submit" class="w-full bg-primary hover:bg-blue-600 text-white font-medium py-2 rounded-lg transition shadow-lg shadow-blue-500/20">
        ...
        <div class="bg-card p-6 rounded-2xl border border-gray-800 shadow-lg">
            ...
            <form method="POST" action="index.php" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="channel_description" rows="2" class="w-full bg-dark border border-gray-700 rounded p-2 text-white outline-none focus:border-primary"><?= htmlspecialchars($settings['channel_description'] ?? '') ?></textarea>
                </div>
                ...
                <button type="submit" class="w-full bg-gray-700 hover:bg-gray-600 text-white font-medium py-2 rounded-lg transition">Update Settings</button>
            </form>
        </div>

        <div class="bg-card p-6 rounded-2xl border border-gray-800 shadow-lg">
            <h2 class="text-lg font-bold text-white border-b border-gray-700 pb-4 mb-4 flex items-center gap-2">
                <i class="fa-solid fa-lock text-primary"></i> Security
            </h2>
            <?php 
                $admin_id = $_SESSION['admin_id'];
            ...
            <form method="POST" action="index.php">
                ...
                <div class="mb-6">
                    <label class="block text-sm text-gray-400 mb-1">Old Password (Required to save changes) *</label>
                    <input type="password" name="old_password" required class="w-full bg-dark border border-gray-700 rounded p-2 text-white outline-none focus:border-primary">
                </div>
                <button type="submit" class="w-full bg-red-500/10 hover:bg-red-500/20 text-red-500 border border-red-500/20 font-medium py-2 rounded-lg transition">Update Credentials</button>
            </form>
        </div>
    </div>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <div class="w-16 h-10 bg-gray-900 rounded overflow-hidden flex-shrink-0 relative">
                                        <?php 
                                        $t_src = '';
                                        if($v['thumbnail_type'] == 'upload') $t_src = '../upload/'.$v['thumbnail_upload'];
